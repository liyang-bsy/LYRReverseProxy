...
public class Proxy extends Task {
	...
	public Proxy(SocketChannel socketChannel) {
		this.source = socketChannel;
		this.timeout = CoreDef.MINUTE;
		log.debug("来源IP:" + source.socket().getInetAddress().getHostAddress());
	}
	...
	public void exec() {
		try {
			...
			reader.debug = source.socket().getInetAddress().getHostAddress();
	...
	public void close() {
		if(isClosed.compareAndSet(false, true)) {
			...
			log.debug("断开IP:" + source.socket().getInetAddress().getHostAddress());
			try {
