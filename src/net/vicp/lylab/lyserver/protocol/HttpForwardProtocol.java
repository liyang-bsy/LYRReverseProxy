...
public class HttpForwardProtocol extends NonCloneableBaseObject implements Protocol {
	...
	{
		Protocol p = new HttpForwardProtocol();
			...
			if (!Utils.inList(headFilters, key)) {
				if (pairs.length == 1) {
					if (!key.contains("HTTP"))
						throw new LYException("No \"HTTP\" was found in head?");
					httpRequest.addHead(key.trim(), null);
				} else {
					value = raw.split("^[^:\\s]*:{1,1}")[1];
	...
	public int validate(byte[] bytes, int offset, int len) {
		return Algorithm.KMPSearch(bytes, HttpRequestHead.doubleSpliter, offset);
	}
